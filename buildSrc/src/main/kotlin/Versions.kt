object Versions {
    const val KOTLIN = "1.7.20"
    const val GRADLE = "7.3.1"
    const val KOTLINX_KOVER = "0.6.1"

    const val KSP = "1.7.20-1.0.7"

    const val KOTLIN_CORE_KTX = "1.9.0"
    const val KOTLINX_DATETIME = "0.4.0"
    const val KOTLIN_COROUTINES = "1.6.4"
    const val KTOR_CLIENT = "2.1.2"

    const val GOOGLE_MATERIAL = "1.8.0-alpha01"
    const val GOOGLE_PLAY_APP_UPDATE = "2.0.0"
    const val PALETTE_KTX = "1.0.0"

    const val COMPOSE_COMPILER = "1.3.2"
    const val COMPOSE = "1.3.0-rc01"
    const val COMPOSE_MATERIAL3 = "1.0.0-rc01"
    const val ACTIVITY_COMPOSE = "1.7.0-alpha01"
    const val NAVIGATION_COMPOSE = "2.6.0-alpha02"
    const val LIFECYCLE_RUNTIME_KTX = "2.6.0-alpha02"
    const val LIFECYCLE_VIEWMODEL_COMPOSE = "2.6.0-alpha02"

    const val HILT = "2.44"
    const val HILT_COMPILER = "1.0.0"
    const val HILT_NAVIGATION_COMPOSE = "1.0.0"

    const val DATASTORE = "1.0.0"
    const val ROOM = "2.5.0-beta01"
    const val ACCOMPANIST = "0.26.5-rc"

    const val THIRD_SUNCALC = "3.5"

    const val TEST_JUNIT = "4.13.2"
    const val TEST_TRUTH = "1.1.3"
    const val TEST_ANDROIDX_JUNIT = "1.1.4-beta01"
    const val TEST_ANDROIDX_ESPRESSO = "3.5.0-alpha05"
    const val TEST_KOTLIN_COROUTINES = "1.6.4"
    const val TEST_TURBINE = "0.12.0"
    const val TEST_ROBOLECTRIC = "4.9"
    const val TEST_ANDROIDX_CORE_KTX = "1.5.0-beta01"
    const val TEST_ANDROIDX_CORE_TESTING = "2.1.0"
}
